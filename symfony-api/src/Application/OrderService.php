    ...
    public function place(Cart $cart, ?User $user, array $data): StoreOrder
    {
        return $this->em->wrapInTransaction(function () use ($cart, $user, $data) {
            ...
            foreach ($cart->getItems() as $item) {
                ...
                if (!$variant->getProduct()->isActive() || $variant->getStock() < $item->getQuantity()) throw new DomainException("Product variant {$variant->getId()} is unavailable.");
                $subtotal += ($variant->getPriceCents() ?? $variant->getProduct()->getPriceCents()) * $item->getQuantity();
            }
            ...
            $this->em->persist(new OrderAddress($order, $data['first_name'], $data['last_name'], $data['line1'], $data['line2'] ?? null, $data['city'], $data['postal_code'], $data['country']));
            foreach ($cart->getItems()->toArray() as $cartItem) {
                $this->em->persist(new OrderItem($order, $cartItem->getVariant(), $cartItem->getQuantity()));
