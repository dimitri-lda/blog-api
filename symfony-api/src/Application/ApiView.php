...
final class ApiView
{
    ...
    {
        return ['id' => $user->getId(), 'name' => $user->getName(), 'email' => $user->getEmail(), 'roles' => $user->getRoles(), 'email_verified' => $user->isEmailVerified()];
    }

    public static function category(Category $category, ?int $count = null): array
    {
        $data = ['id' => $category->getId(), 'name' => $category->getName(), 'slug' => $category->getSlug(), 'image_url' => $category->getImageUrl()];
        if ($count !== null) $data['products_count'] = $count;
        return $data;
    }
    ...
    public static function cart(Cart $cart): array
    {
        $items = array_map(function ($item) {
            ...
            $price = $variant->getPriceCents() ?? $variant->getProduct()->getPriceCents();
            return ['id' => $item->getId(), 'quantity' => $item->getQuantity(), 'variant_id' => $variant->getId(), 'variant_name' => $variant->getName(), 'name' => $variant->getProduct()->getName(), 'image_url' => $variant->getProduct()->getImageUrl(), 'unit_price_cents' => $price, 'line_total_cents' => $price * $item->getQuantity()];
    ...
    public static function order(StoreOrder $order, bool $details = true): array
    {
        ...
        if ($details) {
            $data['address'] = $order->getAddress()?->toArray();
            $data['items'] = array_map(fn($item) => $item->toArray(), $order->getItems()->toArray());
            $data['customer'] = $order->getUser() ? self::user($order->getUser()) : null;
        }
        return $data;
    }
}
