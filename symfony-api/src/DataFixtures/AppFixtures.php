...
final class AppFixtures extends Fixture
{
    ...
    {
        foreach([['Test User','test@example.com',['ROLE_USER']],['Store Admin','admin@example.com',['ROLE_ADMIN']],['Super Admin','superadmin@example.com',['ROLE_SUPERADMIN']]] as [$name,$email,$roles]){$user=new User($name,$email);$user->setRoles($roles);$user->acceptTerms();$user->verifyEmail();$user->setPassword($this->hasher->hashPassword($user,'password'));$manager->persist($user);}
        $categoryData=[
        ...
        ];$categories=[];foreach($categoryData as $slug=>[$name,$image]){$categories[$slug]=new Category($name,$slug,$image);$manager->persist($categories[$slug]);}
        $products=[
            ...
            ['fitness','Move Training Mat','Sportivo','move-training-mat',4900,'A grippy, comfortable mat for your home and studio practice.','https://images.unsplash.com/photo-1592432678016-e910b452f9a2?auto=format&fit=crop&w=900&q=85',true],
            ...
            ['fitness','Kettlebell 12kg','Sportivo','kettlebell-12kg',5900,'A durable cast iron kettlebell for full-body strength sessions.','https://images.unsplash.com/photo-1517963879433-6ad2b056d712?auto=format&fit=crop&w=900&q=85',false],
        ...
        ];foreach($products as [$category,$name,$brand,$slug,$price,$description,$image,$featured]){$product=new Product($categories[$category],$name,$slug,$brand,$description,$price,$image,$featured);$manager->persist($product);$manager->persist(new ProductVariant($product,'One size',strtoupper($slug).'-ONE',$price,24));}
        $manager->flush();
    }
}
