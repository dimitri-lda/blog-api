...
const tone = { pending_payment: 'bg-amber-100 text-amber-800', paid: 'bg-blue-100 text-blue-800', processing: 'bg-violet-100 text-violet-800', shipped: 'bg-cyan-100 text-cyan-800', delivered: 'bg-lime-100 text-lime-800', cancelled: 'bg-slate-200 text-slate-700', refunded: 'bg-rose-100 text-rose-800' };
...
function StatusSelect({ value, onChange, statuses }) {
    ...
    return <div ref={root} className="relative w-full sm:w-52">
        <button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(current => !current)} className={`flex w-full items-center justify-between rounded-xl bg-slate-100 px-4 py-3 text-left text-sm font-semibold transition ${open ? 'bg-slate-200 ring-2 ring-lime-400' : 'hover:bg-slate-200'}`}>
            <span className="truncate">{selected}</span><span className={`ml-3 text-slate-400 transition ${open ? 'rotate-180' : ''}`}>⌄</span>
        </button>
        {open && <div role="listbox" className="absolute left-0 right-0 z-30 mt-2 max-h-72 overflow-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-900/10">
