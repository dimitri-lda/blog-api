...
function StatusSelect({ value, onChange, statuses }) {
    ...
    return <div ref={root} className="relative w-full md:w-48">
        <button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(current => !current)} className={`flex w-full items-center justify-between rounded-xl bg-white px-4 py-3 text-left text-sm font-semibold shadow-sm transition ${open ? 'ring-2 ring-lime-400' : 'hover:bg-slate-50'}`}>
            <span className="truncate">{label(value)}</span><span className={`ml-3 text-slate-400 transition ${open ? 'rotate-180' : ''}`}>⌄</span>
        </button>
        {open && <div role="listbox" className="absolute bottom-full left-0 right-0 z-30 mb-2 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-900/10">
...
export default function Show({ order, nextStatuses }) {
    ...
    return <AdminLayout title={order.number}>
        ...
        <section className="mt-5 rounded-2xl bg-white p-6 shadow-sm"><h2 className="text-lg font-black">Items <span className="font-normal text-slate-400">({order.items.length})</span></h2><div className="mt-5 divide-y divide-slate-100">{order.items.map(item => <div key={item.id} className="grid gap-3 py-5 sm:grid-cols-[1fr_140px_100px_140px] sm:items-center"><div><p className="font-bold">{item.name}</p><p className="mt-1 text-sm text-slate-500">{item.variant_name || 'One size'}</p></div><p className="text-sm text-slate-500">Qty {item.quantity}</p><p className="text-sm text-slate-500">{money(item.unit_price_cents, order.currency)}</p><p className="font-bold sm:text-right">{money(item.line_total_cents, order.currency)}</p></div>)}</div></section>
    </AdminLayout>;
}
