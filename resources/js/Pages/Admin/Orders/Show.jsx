...
function StatusSelect({ value, onChange, statuses }) {
    ...
    return <div ref={root} className="relative w-full md:w-48">
        <button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(current => !current)} className={`flex w-full items-center justify-between rounded-xl bg-white px-4 py-3 text-left text-sm font-semibold shadow-sm transition ${open ? 'ring-2 ring-blue-400' : 'hover:bg-slate-50'}`}>
...
export default function Show({ order, nextStatuses }) {
    ...
    return <AdminLayout title={order.number}>
        ...
        <div className="mt-8 grid gap-5 xl:grid-cols-2"><section className="rounded-2xl bg-white p-6 shadow-sm"><h2 className="text-lg font-black">Order details</h2><div className="mt-6 grid grid-cols-2 gap-6"><Field name="Status" value={label(order.status)} /><Field name="Email" value={order.email} /><Field name="Phone" value={order.phone} /><Field name="Delivery" value={order.delivery_method} /><Field name="Net subtotal" value={order.net_subtotal_cents == null ? '—' : money(order.net_subtotal_cents, order.currency)} /><Field name="VAT" value={order.tax_cents == null ? '—' : money(order.tax_cents, order.currency)} /><Field name="Gross subtotal" value={money(order.subtotal_cents, order.currency)} /><Field name="Delivery cost" value={money(order.delivery_cents, order.currency)} /><Field name="Total" value={money(order.total_cents, order.currency)} /><Field name="Currency" value={order.currency} /><Field name="Market" value={order.market} /><Field name="Exchange rate" value={order.exchange_rate} /><Field name="VAT rate" value={order.vat_rate_basis_points == null ? '—' : `${order.vat_rate_basis_points / 100}%`} /></div></section><section className="rounded-2xl bg-white p-6 shadow-sm"><h2 className="text-lg font-black">Customer & delivery</h2><div className="mt-6 grid grid-cols-2 gap-6"><Field name="Customer" value={order.customer?.name} /><Field name="Customer email" value={order.customer?.email} /><Field name="First name" value={order.address?.first_name} /><Field name="Last name" value={order.address?.last_name} /><Field name="Address" value={[order.address?.line1, order.address?.line2].filter(Boolean).join(', ')} /><Field name="City" value={[order.address?.postal_code, order.address?.city].filter(Boolean).join(' ')} /><Field name="Country" value={order.address?.country} /></div></section></div>
