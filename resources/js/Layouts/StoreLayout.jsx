...
function Preferences() {
    ...
    const closePreferences = (nextCountry = country, nextLocale = locale) => {
        if (nextCountry !== market?.country || nextLocale !== market?.locale) {
            router.post(route('market.preferences.update'), { country: nextCountry, locale: nextLocale }, { preserveScroll: true });
        }
        setExpanded(false);
    };
    ...
    useEffect(() => {
        const closeOnOutsideClick = event => {
            if (expanded && preferencesRef.current && !preferencesRef.current.contains(event.target)) closePreferences();
        };
        const closeOnEscape = event => { if (expanded && event.key === 'Escape') closePreferences(); };
    ...
    }, [country, expanded, locale, market?.country, market?.locale]);

    return <div ref={preferencesRef} className="relative w-full max-w-xs">
        <button type="button" onClick={() => expanded ? closePreferences() : setExpanded(true)} aria-expanded={expanded} aria-controls="market-preferences" className="flex w-full items-center justify-between rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-left text-sm font-semibold text-white transition hover:border-slate-500 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-400">
        ...
        {expanded && <div id="market-preferences" className="absolute bottom-[calc(100%+0.5rem)] right-0 z-50 w-full rounded-xl border border-slate-700 bg-slate-900 p-2 shadow-xl shadow-black/30">
            ...
            <select aria-label={t('market')} value={country} onChange={event => setCountry(event.target.value)} className="w-full rounded-lg border-0 bg-slate-800 px-3 py-2.5 text-sm text-white focus:ring-2 focus:ring-blue-400"><optgroup label="Belarus & Poland"><option value="BY">{countries.BY?.[locale] ?? 'Belarus'}</option><option value="PL">{countries.PL?.[locale] ?? 'Poland'}</option></optgroup><optgroup label={t('eu_global')}>{Object.entries(countries).filter(([code]) => !['BY', 'PL'].includes(code)).map(([code, names]) => <option key={code} value={code}>{names[locale] ?? names.en}</option>)}</optgroup></select>
            <select aria-label={t('language')} value={locale} onChange={event => { setLocale(event.target.value); closePreferences(country, event.target.value); }} className="mt-2 w-full rounded-lg border-0 bg-slate-800 px-3 py-2.5 text-sm text-white focus:ring-2 focus:ring-blue-400"><option value="en">English</option><option value="ru">Русский</option><option value="pl">Polski</option></select>
        </div>}
    </div>;
}
