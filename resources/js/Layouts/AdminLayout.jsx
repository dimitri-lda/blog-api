        ...
        <div className="min-h-screen lg:grid lg:grid-cols-[248px_1fr]">
            <aside className="bg-slate-950 px-5 py-5 text-white lg:min-h-screen lg:px-6 lg:py-8">
                <div className="flex items-center justify-between lg:block">
                    <Link href={route('admin.orders.index')} className="text-xl font-black tracking-tight"><span className="text-lime-400">sport</span>ivo <span className="ml-1 text-xs font-semibold uppercase tracking-[.2em] text-slate-400">admin</span></Link>
                ...
                <nav className="mt-6 flex gap-2 lg:mt-12 lg:block lg:space-y-2">
                    <Link href={route('admin.orders.index')} className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${route().current('admin.orders.*') ? 'bg-lime-400 text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>Orders</Link>
                </nav>
            ...
            <main className="min-w-0 px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
                {flash?.success && <div className="mb-6 rounded-xl bg-lime-100 px-4 py-3 text-sm font-semibold text-lime-900">{flash.success}</div>}
